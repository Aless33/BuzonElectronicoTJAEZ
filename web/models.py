...
class TipoPromocion(models.TextChoices):
    ...
    EXPEDIENTE_RAG_INICIAL = (
        'EXPEDIENTE_RAG_INICIAL',
        'Expediente de Resp. Admvas (RAG) Inicial',
    )
    EXPEDIENTE_RAG = (
        'EXPEDIENTE_RAG',
        'Expediente de Resp. Admvas (RAG)',
    )
    OTROS = 'OTROS', 'Otros'
...
class BuzonBase(models.Model):
    ...
    def __str__(self):
        tipo = self.get_tipo_promocion_display()
        return f"#{self.pk} - {tipo} ({self.correo_electronico})"


class ExpedienteMixin(models.Model):
    """
    Campos extra compartidos por: Contestación, Alegatos,
    Informe de Autoridad, Recurso, Incidente, Amparo y Expediente RAG.
    """
...
class BuzonConExpedinteBase(ExpedienteMixin, BuzonBase):
    """
    Base intermedia abstracta para los tipos que comparten exactamente
    los mismos campos extra sin agregar nada más.
    """
...
class Etiqueta(models.Model):
    ...
    digito_verificador = models.CharField(
        max_length=6,
        default=_generar_digito_verificador,
    )
    ...
    def __str__(self):
        return (
            ...
            f"Sobre {self.numero_sobre} [{self.estado}]"
        )
