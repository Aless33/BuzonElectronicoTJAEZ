...
class BaseWidgets:
    ...
    base_widgets = {
        'tipo_promocion': forms.Select(
            attrs={
                'class': 'form-select',
                'id': 'id_tipo_promocion'}),
        'correo_electronico': forms.EmailInput(
            attrs={
                'class': 'form-control'}),
        'numero_sobres': forms.NumberInput(
            attrs={
                'class': 'form-control',
                'min': 1}),
    }
...
class CorreoConfirmacionMixin:
    ...
    def clean(self):
        ...
        numero_sobres = cleaned_data.get('numero_sobres')
        ...
        if numero_sobres is not None and numero_sobres < 1:
            self.add_error(
                'numero_sobres',
                'El número de sobres debe ser mayor o igual a 1.'
            )
...
class BuzonInformeAutoridadForm(
        CorreoConfirmacionMixin,
        BaseWidgets,
        forms.ModelForm):
    class Meta:
